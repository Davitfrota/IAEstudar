...
Headers de resposta: `Content-Type: application/x-ndjson`. Uma linha JSON por evento:
...
- Histórico limitado a **40** mensagens
...
Confirmação: para `generate_schedule` / `generate_form`, o agente deve chamar com `confirmed=false`, explicar o preview e só persistir após o usuário confirmar (`confirmed=true`). Com pending, o loop para após o follow-up textual.
...
- Preview exige doc ≥ 50 chars em `content_text`
- Persistência chama Anthropic e cria questões tipadas por form (`qa` / `multiple_choice` / `open`)
